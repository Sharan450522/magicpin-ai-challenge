...
# Load project .env so LLM_API_KEY / LLM_PROVIDER match the bot (optional)
try:
    from dotenv import load_dotenv

    load_dotenv(DATASET_DIR.parent / ".env")
except ImportError:
    pass
_k = os.environ.get("LLM_API_KEY", "").strip()
if _k:
    LLM_API_KEY = _k
_p = os.environ.get("LLM_PROVIDER", "").strip().lower()
if _p:
    LLM_PROVIDER = _p
_m = os.environ.get("LLM_MODEL", "").strip()
if _m:
    LLM_MODEL = _m
_b = os.environ.get("BOT_URL", "").strip().rstrip("/")
if _b:
    BOT_URL = _b
...
class GroqProvider(LLMProvider):
    def __init__(self, api_key: str, model: str = ""):
        ...
        self.model = (model or "").strip() or "llama-3.3-70b-versatile"
    ...
    def complete(self, prompt: str, system: str = None) -> str:
        ...
        # Cloudflare 1010: urllib default has no User-Agent — Groq/WAF blocks it
        groq_headers = {
        ...
        try:
            ...
            try:
                ...
                detail = str(e.reason)
            raise RuntimeError(
                f"Groq HTTP {e.code}: {detail}\n"
                f"Check LLM_API_KEY at https://console.groq.com/keys (no quotes/spaces). "
                f"If the model was retired, set LLM_MODEL in .env (e.g. llama-3.1-8b-instant)."
            ) from e
        data = json.loads(resp.read().decode("utf-8"))
        return data["choices"][0]["message"]["content"]
...
class JudgeSimulator:
    ...
    def _warmup(self) -> bool:
        ...
        for slug, cat in self.dataset.categories.items():
            ...
            detail = ""
            if status == "FAIL":
                detail = f" — {err}" if err else f" — {data.get('reason', data) if isinstance(data, dict) else data}"
            print(f"  [{status}] category/{slug}{detail}")
        ...
        for mid, m in list(self.dataset.merchants.items())[:5]:
            ...
            if status == "FAIL":
                detail = f" — {err}" if err else f" — {data.get('reason', data) if isinstance(data, dict) else data}"
            print(f"  [{status}] merchant/{short_id}{detail}")
    ...
    def _auto_reply(self) -> bool:
        ...
        # Same id for all 4 turns; unique per judge run so server memory does not leak streaks
        conv_auto = f"judge_conv_auto_{uuid.uuid4().hex[:12]}"
        for i in range(1, 5):
